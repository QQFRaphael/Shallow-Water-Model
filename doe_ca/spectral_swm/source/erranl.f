...
C                                                                              
C CALLED BY: STSWM
C CALLS: AGSETI, ANLYTC, ANOTAT, EZXY, PLOTS, WEIGHT
C
C REVISIONS:
...
C
      REAL PL1G(NGRPHS),VL1G(NGRPHS)
      SAVE PL1G, VL1G
      REAL VL2G(NGRPHS), PL2G(NGRPHS)
      SAVE VL2G, PL2G
      REAL PINFG(NGRPHS),VINFG(NGRPHS),TME(NGRPHS)
      SAVE PINFG, VINFG, TME
C
C     MINIMUM, MAXIMUM, MEAN AND VARIANCE
C
      REAL PMING(NGRPHS),PMAXG(NGRPHS)
      SAVE PMING, PMAXG
      REAL PAVGG(NGRPHS),PVARG(NGRPHS)
      SAVE PAVGG, PVARG
C
C     GRID WEIGHTS
C
...
C
      REAL UANL(NLON+2,NLAT),VANL(NLON+2,NLAT),
     $            PANL(NLON+2,NLAT)
      REAL DANL(NLON+2,NLAT),ZANL(NLON+2,NLAT)
C
...
C
  ...
  122 FORMAT (' HEIGHT ERROR',/,
     ...
     $        ' L(INF) = ', 1PE16.9)
      ...
      write(51,'(3e15.8)') PL1G(L2CTR),PL2G(L2CTR),PINFG(L2CTR)
C
  ...
  123 FORMAT (' VECTOR WIND ERROR',/,
     ...
     $        ' L(INF) = ', 1PE16.9)
C
C     WRITE H_MIN,H_MAX,H_MEAN,H_VAR NORMALIZED DATA
C
      WRITE (6,128) PMING(L2CTR), PMAXG(L2CTR)
  128 FORMAT (' NORMALIZED HEIGHT MIN./MAX.  = ',1PE16.9,'/',1PE16.9)
C
      WRITE (6,130) PAVGG(L2CTR), PVARG(L2CTR)
  130 FORMAT (' NORMALIZED HEIGHT AVG./VAR.  = ',1PE16.9,'/',1PE16.9)
C
...
C                                                                               
  ...
  260    CONTINUE
C
C        PLOTS OF ERROR (COMPUTED - ANALYTIC)
