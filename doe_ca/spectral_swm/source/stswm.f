...
C        OUTPTP, PLOTS, SHTRNS, SPCANL, STEP, TIMING,
C        TPRINT, WEIGHT, ZD
C
C REVISIONS:
...
C     (SET TEST CASE NUMBER 'ICOND = 7' IN NAMELIST INPUT !)
C
      LOGICAL LDIFF
      PARAMETER (LDIFF = .FALSE.)
C
C     FLAG FOR TIMING MEASUREMENTS
C
      LOGICAL LTIME
      PARAMETER (LTIME = .FALSE.)
C
C------ Local Variables ------------------------------------------------
C
...
C     TIMING VARIABLE
C
      DOUBLE PRECISION TWALLA(0:NGRPHS), TWALLB(0:NGRPHS)
...
C
     ...
     $              IFIX(XTAU * 3600.0/DT * (1.0+EPS)))) .EQ. 0
C
C----- Executable Statements -------------------------------------------        
C
C     COMPLETE PROGRAM TIMING
C
      IF (LTIME) THEN
         CALL TIMING(0,TWALLA,TCPUA)
      ENDIF
C
C     COPYRIGHT MESSAGE
C
      WRITE(6,10)
   10 FORMAT(/,' SPECTRAL TRANSFORM SHALLOW WATER MODEL, Version 2.1',
...
C
      LN = 1
C
C     INDEX FOR ENERGETICS ANALYSIS
C
      ECTR = 1
C
C     INDEX FOR ERROR ANALYSIS
C
      L2CTR = 1
C
C     INDEX FOR SPECTRAL COEFFICIENT OUTPUT
C
      OCTR = 1
C
...
C
         DO 1805 I=1,NLON
            UCOS(I,J)   = WS1(I,J) * FAC
            VCOS(I,J)   = WS2(I,J) * FAC
            VLEN  = WS1(I,J)*WS1(I,J)+WS2(I,J)*WS2(I,J)
            IF (VLEN .GT. MAXV) THEN
               MAXV = VLEN
            ENDIF
            IF (FTOPO) THEN
               PHI(I,J,LN) = GRAV * (WS3(I,J) - MOUNT(I,J))
            ELSE
...
C'                                                                     '
      IF (ERRFRQ .LE. TAUE) THEN
         CALL ERRANL(WS3,WS1,WS2,DIV,ZETA,LN,.FALSE.,L2CTR,
     $                    .FALSE.)
      ENDIF
C
C'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
C
      IF ((TAUO .LE. TAUE) .OR. (SPCFRQ .LE. TAUE)) THEN
...
C
      IF (LTIME) THEN
         CALL TIMING(NSTEP,TWALLA,TCPUA)
      ENDIF
C
      IF (LDIFF) THEN
C
...
C
         CALL ANLYTC(LVLS,LN,NSTEP*DT,ICOND,WS1,WS2,WS3,DIV,ZETA)
C
      ELSE
C
...
C
            DO 90 I=1,NLON
               WS1(I,J) = UCOS(I,J) * FAC
               WS2(I,J) = VCOS(I,J) * FAC
...
C
      IF (LTIME) THEN
         CALL TIMING(NSTEP,TWALLB,TCPUB)
      ENDIF
C
...
C
      IF (LTIME) THEN
         CALL TIMING(0,TWALLB,TCPUB)
C
C        OUTPUT TIMING RESULTS
C
         CALL TPRINT(NSTEP,TWALLA,TWALLB,TCPUA,TCPUB)
      ENDIF
C                                                                               
