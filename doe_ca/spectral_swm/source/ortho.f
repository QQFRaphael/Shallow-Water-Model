...
C
      DO 300 NL = 1, NLATB2
         DO 200 IS = 1, NALP
            ALPON(IS) = ALPON(IS) 
     $                  + 2.0*ALP(IS,NL)*ALP(IS,NL)*WTS(NL)
  200    CONTINUE 
  300 CONTINUE
C
      DO 390 JN = 0, NN
            ...
            DO 380 NL = 1, NLATB2
               DO 350 ISO = 0,JN-1
                  ...
                  ELSE
                     COEFF2 = -1.0
                  ENDIF
                  TMP(ISO) = TMP(ISO) + (1.0+COEFF1*COEFF2) 
     $                    * ALP(LDIAG(ISO,2)+JM,NL)
     $                    * ALP(IS+JM,NL)* WTS(NL)
  350          CONTINUE
  380       CONTINUE
C
...
C
  ...
  500 CONTINUE
C
      CALL CPCNRC(PLOTON(0,0),MM+1,MM+1,KK+1,0.0,0)
      CALL LABTOP('Orthonormality Error',0.015)
      CALL FRAME
      CALL CPCNRC(PLOTOG(0,0),MM+1,MM+1,KK+1,0.0,0)
