...
C
C Modified for non-equidistant gridpoint plots as part
...
C#
C#    MODIFIED NCAR GRAPHICS ROUTINE TO HANDLE GAUSSIAN GRID
C#    USING ROUTINES CPMPXY AND CPMVXY 
C#
...
C                        in July 1984.
C                        Revised in October, 1990, to use the CONPACK
C                        routine CPMPXY for map transformations,
C                        and the routine CPMVXY for vector mapping.
C
...
C
C The functions FX and FY are replaced by the subroutine 
C CPMPXY, which is part of the CONPACK package, the functions
C MXF and MYF by the subroutine CPMVXY 
C ---------------------------------------------------------------------
C
C DECLARATIONS -
C
...
C
      INTEGER LU, LV, N, M, NSET, LENGTH, ISPV
      REAL FLO, HI
      REAL       U(LU,N)    ,V(LV,N)    ,SPV(2)
      CHARACTER*10    LABEL
...
C                                 (REPLACED BY CPMPXY)
C
C                        MXF      RETURNS THE X-COORDINATE OF THE VECTOR
C                                 HEAD. (REPLACED BY CPMVXY)
C
...
C     FY(XX,YY) = YY
      REAL XX,YY,DIST
      INTEGER MM,NN,INCXX,INCYY,LENN
      REAL HAA,XX1,XX2,YY1,YY2,XX3,XX4,YY3,YY4
      REAL UU,VV,SCALEX,SCALEY,VLAB
      INTEGER II,JJ
      REAL R1MACH
      INTEGER MX,MY,NX,NY
      REAL GL,HA
      ...
      INTEGER I,J
      REAL SFX, SFY
      INTEGER IOFFDT,IZFLG
      REAL UI,VI,X,Y
      INTEGER MXF,MYF
      INTEGER IER,ICLP,IX,IY,ICN
      REAL XC,YC,CPUX,CPUY
C
...
C #### USING NEW CONPACK FUNCTION CPMPXY ####
      CALL CPMPXY(3,1.,1.,XOTP,YOTP)
      CALL FL2INT(XOTP,YOTP,MX,MY)
C ### CALL FL2INT(FX(1.,1.),FY(1.,1.),MX,MY)
      CALL CPMPXY(3,REAL(1+INCX),REAL(1+INCY),XOTP,YOTP)
      CALL FL2INT(XOTP,YOTP,LX,LY)
C ### CALL FL2INT(FX(REAL(1+INCX),REAL(1+INCY)),
C ###+            FY(REAL(1+INCX),REAL(1+INCY)),LX,LY)
      LEN = SQRT((REAL(MX-LX)**2+REAL(MY-LY)**2)/2.)
C
...
C
  ...
  121       X = I
            Y = J
C ### REPLACE BY CONPACK ROUTINE CPMPXY ####
            CALL CPMPXY(3,X,Y,XOTP,YOTP)
C
C           OUT OF RANGE VECTOR BASE
C
            IF (XOTP .NE. 1.0E12) THEN
               CALL FL2INT(XOTP,YOTP,MX,MY)
C ###          CALL FL2INT(FX(X,Y),FY(X,Y),MX,MY)
C ### REPLACE BY NEW MAPPING ROUTINE CPMVXY FOR VECTORS ###
               CALL CPMVXY(3,X,Y,UI,VI,SFX,SFY,MX,MY,MXF,MYF)
               LX = MAX0(1,MXF)
               LY = MAX0(1,MYF)
C ###          LX = MAX0(1,MXF(X,Y,UI,VI,SFX,SFY,MX,MY))
C ###          LY = MAX0(1,MYF(X,Y,UI,VI,SFX,SFY,MX,MY))
               IZFLG = 1
               IF (ILAB .NE. 0) 
     &            CALL ENCD(VLAB(UI,VI,I,J),ASH,LABEL,NC,IOFFDT)
