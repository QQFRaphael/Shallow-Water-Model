...
C*    relation LRM = ((MM+1)*(KK+1)-(MM**2+MM)/2).  The length of      *        
C*    each row is stored in the array LROW(0:KK,1).                    *        
C*    This is also where we store EPSIL which is used in a             *        
...
C*                                                                     *        
      REAL CMN(LRM), DMN(LRM), EMN(LRM), EPSIL(NALP)        
      INTEGER LROW(0:KK,2)
C
C     TEMPORARIES
C
...
C                                                                               
   ...
   80 CONTINUE                                                                  
C
C     COMPUTE M INDEX ARRAY (SINCE CRAY HAS SLOW INTEGER DIVISION !)
C
      DO 90 JM=1,MM+1
         REALM(JM) = REAL(JM-1)
   90 CONTINUE 
