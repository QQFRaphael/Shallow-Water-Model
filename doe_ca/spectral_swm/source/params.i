...
C     NGRPHS is the amount of storage allocated for time history data
C     LVLS is the number of time levels for prognostic variables.        
C                                                                       
      INTEGER NFC,LMN,LRM,NALP,NLATB2
      PARAMETER (NFC=(NLON+2)/2)
      PARAMETER (NLATB2=NLAT/2)
      PARAMETER (LMN=MM+NN-KK, NALP=(MM+1)*(NN+1)-(LMN**2+LMN)/2)           
      PARAMETER (LRM=(MM+1)*(KK+1)-(MM*MM+MM)/2)                              
      INTEGER NGRPHS,LVLS
